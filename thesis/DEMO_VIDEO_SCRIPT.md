...
> **Тривалість:** ~4:32 при темпі 2.3 слова/сек.
...
## 📋 Загальна карта (6 сцен ≈ 4:32)

| # | Що показуємо | URL / дія | Сек |
|---|---|---|---|
| 1 | `/benchmark` техніка + вступ | відкрити `/benchmark?domain=tech` | 61 |
| 2 | `/benchmark/selection` техніка + профілі по доменах | вкладка «Вибір моделі» | 75 |
| 3 | **Повзунок затримки → рекомендація → E5-base** | перетягнути повзунок | 50 |
| 4 | `/` техніка + BGE-M3 + запит про churn | вкладка «Пошук», ввести | 28 |
| 5 | Той самий запит, **BM25** | клік плашки моделі | 17 |
| 6 | `/benchmark/explorer/q6` + завершення | вкладки «Бенчмарк» → «По запитах» | 41 |
| | **РАЗОМ** | | **~4:32** |
...
### Сцена 1/6 · `0:00 – 1:01` · `/benchmark` техніка

**Дія:** відкрий `http://127.0.0.1:5000/benchmark?domain=tech`.

**Голос:**

> Питання роботи: **серед п'яти embedding-моделей — яку обрати для конкретної бази знань?** Стартуємо з виміряних результатів на технічному домені — порівняння за чотирма метриками якості пошуку. **nDCG** показує, наскільки правильно ранжована видача; **MRR** — як рано з'являється перший релевантний результат; **Recall** — яку частку всіх релевантних документів знайдено; **Precision** — яка частка видачі справді релевантна. П'ятий критерій — латентність, час відповіді в мілісекундах. Виміряно 5 моделей по 100 запитах. Лідер за nDCG — **BGE-M3 з 0.6722**. Далі Qwen3 з 0.6325, потім E5-base, BM25 і nomic. Але «лідер за однією метрикою» — не «найкращий вибір»: треба враховувати швидкодію і ресурси. Тому є окрема сторінка вибору моделі.

<img src="demo_screenshots/07_benchmark_tech.png" alt="бенчмарк техніка" width="650" />

---

### Сцена 2/6 · `1:01 – 2:16` · `/benchmark/selection` техніка — центральна частина демо
...
### Сцена 3/6 · `2:16 – 3:06` · повзунок латентності → інша рекомендація — ключовий момент
...
### Сцена 4/6 · `3:06 – 3:34` · `/` пошук техніка + BGE-M3 + запит про churn
...
### Сцена 5/6 · `3:34 – 3:51` · BM25 на тому самому запиті

**Дія:** клікни плашку моделі **BM25** під полем пошуку. Запит залишиться той самий.

**Голос:**

> Для порівняння перемикаю на BM25 — одну з моделей з нижчою інтегральною оцінкою. Той самий запит. BM25 теж знаходить **ту саму** роботу, але повертає менш корисний фрагмент: абстракт зі списком літератури замість змістового тексту.

<img src="demo_screenshots/03_tech_bm25_churn.png" alt="техніка BM25 запит churn" width="650" />

---

### Сцена 6/6 · `3:51 – 4:32` · `/benchmark/explorer/q6` + завершення
...
>
> На цьому висновки роботи підтверджено: оптимальна модель залежить і від домену, і від пріоритетів користувача; розроблений застосунок дає інструмент для обґрунтованого багатокритеріального вибору.
...
| Сцена | Слова | Сек |
|---|---|---|
| 1. Бенчмарк техніка + вступ | 140 | 61 |
| 2. Вибір моделі техніка + профілі | 175 | 75 |
| 3. Повзунок латентності | 115 | 50 |
| 4. Пошук BGE + churn | 65 | 28 |
| 5. Пошук BM25 + churn | 40 | 17 |
| 6. Деталі по q6 + завершення | 95 | 41 |
| **Сума** | **~630** | **~272 с (≈ 4:32)** |

**Спільний бюджет зі слайдами:**
- Слайди: 1221 слово ≈ 9 хв при темпі 2.3 слова/сек
- Демо: ~4:32
- **Разом ≈ 13:32** ✓ Запас часу ~1:30 на паузи/перехід.

Якщо все одно затиснутий по часу — найбезболісніше скорочується сцена 5 (контраст із BM25). Вона підкріплює основну ідею, але не несе нової концепції.
