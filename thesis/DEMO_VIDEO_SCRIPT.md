...
### Сцена 2/9 — `/benchmark` техніка (~56 с)
...
### Сцена 3/9 — `/benchmark/selection` техніка (~50 с) — центральна частина демо
...
### Сцена 4/9 — повзунок латентності → інша рекомендація (~50 с) — ключовий момент
...
### Сцена 5/9 — `/benchmark` юриспруденція (~22 с)
...
### Сцена 6/9 — `/` пошук техніка + BGE-M3 + запит про churn (~28 с)
...
### Сцена 7/9 — BM25 на тому самому запиті (~30 с)
...
### Сцена 8/9 — `/benchmark/explorer/q6` (~33 с)
