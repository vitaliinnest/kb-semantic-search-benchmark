...
> Для порівняння перемикаю на BM25 — одну з моделей з нижчою інтегральною оцінкою. Той самий запит. BM25 теж знаходить **ту саму** роботу, але повертає менш корисний фрагмент: абстракт зі списком літератури замість змістового тексту. Так підтверджується практичний сенс MCDA-рекомендації — обрана модель дає кращий результат на реальному запиті.
...
> Висновки роботи підтверджено: оптимальна модель залежить і від домену, і від пріоритетів користувача; розроблений застосунок дає інструмент для обґрунтованого багатокритеріального вибору. Дякую за увагу — готовий відповісти на запитання.
