...
> **Тривалість:** ~3–4 хв залежно від темпу і чи зачитуєш фрагменти у [квадратних дужках].
> Разом зі слайдами вкладаємось у 15-хвилинне вікно з запасом ~1–2 хв.
>
> **Як працювати з цим файлом:** читай вголос **звичайний текст**. Фрагменти у `[квадратних дужках]` — опціональні поглиблення. Якщо темп комфортний — додавай їх; якщо біжиш по часу — пропускай.
...
## 📋 Загальна карта (8 секцій ≈ 3:30)

| # | Що показуємо | URL / дія | Сек (з опц.) |
|---|---|---|---|
| 1 | Hero | `/` | 15 |
| 2 | `/benchmark` Tech | Tab «Benchmark» | 30 |
| 3 | `/benchmark/selection` Tech | Tab «Вибір моделі» | 40 |
| 4 | **Тягнеш Latency повзунок → рек. → E5-base** | drag слайдер | 40 |
| 5 | `/benchmark` для Legal | дропдаун домену | 20 |
| 6 | `/` Tech BGE-M3 + churn-запит | Tab «Пошук», ввести | 25 |
| 7 | Той самий запит, **BM25** | клік плашки моделі | 20 |
| 8 | `/benchmark/explorer/q6` | URL або кліком | 25 |
| 9 | Closing | — | 10 |
| | **РАЗОМ** | | **~3:45** |
...
### Section 3 — `/benchmark/selection` Tech (~40 с) — герой демо
...
### Section 4 — слайдер Latency → інша рекомендація (~40 с) — wow-момент
...
### Section 5 — `/benchmark` для Legal (~20 с)
...
### Section 6 — `/` пошук Tech BGE-M3 + churn (~25 с)

**Дія:**
- Tab «Пошук». Перемкни домен назад на **«Техніка / IT»**. Модель — BGE-M3.
- У поле введи: **`прогноз відтоку клієнтів (churn prediction)`**. Натиснути «Шукати».
...
### Section 7 — BM25 на тому самому запиті (~20 с)
...
### Section 8 — `/benchmark/explorer/q6` (~25 с)
...
| Рекомендація не змінюється після слайдеру | Debounce 650 мс. Зачекай 1 секунду перед коментарем. |
...
| Секція | Слова (з [опц.]) | Сек | Без [опц.] |
|---|---|---|---|
| 1. Hero | 68 | 30 | 25 |
| 2. Benchmark Tech | 120 | 52 | 38 |
| 3. Selection Tech | 145 | 63 | 47 |
| 4. Latency slider | 150 | 65 | 50 |
| 5. Benchmark Legal | 65 | 28 | 22 |
| 6. Search BGE churn | 80 | 35 | 28 |
| 7. Search BM25 churn | 80 | 35 | 28 |
| 8. Explorer q6 | 95 | 41 | 33 |
| 9. Closing | 38 | 16 | 16 |
| **Сума** | **~841** | **~366 с (≈ 6:00)** | **~290 с (≈ 4:50)** |

*Чесна оцінка: повний скрипт з опціональними фрагментами — ~6 хв; стислий (без [квадратних дужок]) — ~5 хв.*

**Спільний бюджет із слайдами:**
- Слайди: 1221 слово ≈ 9 хв при темпі 2.3 wps
- Демо стисле: ~5 хв
- **Разом ≈ 14 хв** ✓ Вкладаємось у 15 хв з невеликим запасом.

Якщо хочеш ще коротше демо (~3 хв) — пропускай усі [квадратні дужки] і скорочуй секції 5 та 7 (вони слугують для контрасту, але не несуть нових концепцій).
