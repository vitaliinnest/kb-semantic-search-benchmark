...
NOTES_EDITS: dict[int, list[tuple[str, str]]] = {
    2: [
        ...
        (
            "оцінити моделі за стандартними метриками",
            "оцінити моделі за метриками retrieval-якості",
        ),
        (
            "обґрунтовано обрати оптимальну модель",
            "обрати найкращу модель методом багатокритеріального аналізу",
        ),
    ],
    5: [
        (
            "Для дослідження обрано чотири сучасні моделі.",
            "Для дослідження обрано чотири моделі різних архітектур.",
        ),
    ],
    6: [
        (
            "чотирьох стандартних метриках retrieval-якості",
            "чотирьох метриках retrieval-якості",
        ),
        (
            "Для достовірності — три методи аналізу",
            "Для статистичної значущості — три методи аналізу",
        ),
    ],
    13: [
        (
            "Достовірність підтверджена бутстрепом",
            "Статистична значущість підтверджена бутстрепом",
        ),
    ],
    15: [
        (
    ...
    16: [
        (
            "E5-base — Парето-оптимальна sweet spot: ~70 мілісекунд, утричі швидше за BGE-M3 при незначній втраті якості.",
            "E5-base — Парето-оптимальна: ~70 мілісекунд, утричі швидше за BGE-M3 при якості на ~10% нижчій.",
        ),
        (
            "BM25 — Парето-оптимальна як baseline: менше 3 мілісекунд, але стеля якості ~0.49.",
            "BM25 — Парето-оптимальна як baseline: менше 3 мілісекунд, найвища nDCG — ~0.49.",
        ),
    ],
    17: [
        (
