...
#
# CRITICAL: filename ↔ figure mapping comes from document.xml relationships,
# NOT from imageN.png ↔ fig 5.N. Multiple figures can share one image rId
# (e.g. 5.12+5.21 both use image12). Verified mapping (2026-05-07):
#   image4   -> fig 5.4   (documents tech)
#   image6   -> fig 5.6   (build tech)
#   image7   -> fig 5.7   (search hero tech)
#   image8   -> fig 5.8   (search tech bge)
#   image9   -> fig 5.9   (search legal bge)
#   image10  -> fig 5.10  (search medical bge)
#   image12  -> fig 5.12 + 5.21  (both = benchmark tech)
#   image14  -> fig 5.14 + 5.24  (both = selection tech)
#   image15  -> fig 5.15  (explorer index)  -- NOT fig 5.18!
#   image16  -> fig 5.16  (explorer detail) -- NOT fig 5.19!
#   image17  -> fig 5.17  (explorer drill bge) -- NOT fig 5.20!
#   image18  -> fig 5.18  (search tech bge en)
#   image19  -> fig 5.19  (search tech nomic)
#   image20  -> fig 5.20  (search medical e5)
#   image21  -> fig 5.22  (benchmark legal)
#   image40  -> fig 5.23  (benchmark medical)  -- added 2026-05-07
#   image41  -> fig 5.25  (selection legal)    -- added 2026-05-07
#   image42  -> fig 5.26  (selection medical)  -- added 2026-05-07
SHOTS = [
    ...
    ("image15.png", f"{BASE}/benchmark/explorer?domain=tech",              1400, "5.15 explorer index tech"),
    ("image16.png", f"{BASE}/benchmark/explorer/q1?domain=tech",           1700, "5.16 explorer detail q1"),
    ("image17.png", f"{BASE}/benchmark/explorer?domain=tech&model=" + quote("BGE-M3", safe=""), 1300, "5.17 explorer drill bge"),
    ("image18.png", f"{BASE}/?" + urlencode({
        "domain": "tech", "model": "bge-m3", "top_k": 5,
        "q": "machine learning algorithms overview"
    }), 1500, "5.18 search tech bge en"),
    ("image19.png", f"{BASE}/?" + urlencode({
        "domain": "tech", "model": "nomic", "top_k": 5,
        "q": "sorting algorithms complexity"
    }), 1500, "5.19 search tech nomic"),
    ("image20.png", f"{BASE}/?" + urlencode({
        "domain": "medical", "model": "e5-base", "top_k": 5,
        "q": "першочергова допомога при серцевому нападі"
    }), 1500, "5.20 search medical e5"),
    ("image21.png", f"{BASE}/benchmark?domain=legal",                      1500, "5.22 benchmark legal"),
    ("image40.png", f"{BASE}/benchmark?domain=medical",                    1500, "5.23 benchmark medical"),
    ("image41.png", f"{BASE}/benchmark/selection?domain=legal",            1500, "5.25 selection legal"),
    ("image42.png", f"{BASE}/benchmark/selection?domain=medical",          1500, "5.26 selection medical"),
]
