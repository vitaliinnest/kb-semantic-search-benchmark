# ПІДВИЩЕННЯ ЯКОСТІ СЕМАНТИЧНОГО ПОШУКУ В БАЗАХ ЗНАНЬ ІЗ ВИКОРИСТАННЯМ ВЕКТОРНИХ ПОДАНЬ ТА БЕНЧМАРК-ОЦІНЮВАННЯ

This paper presents an applied study of semantic search quality in a knowledge-base retrieval system with a focus on practical evaluation and result visualization. The implemented workflow includes document preprocessing, chunking, vector indexing, query execution, and benchmark-based assessment in a single reproducible pipeline. Particular attention is paid to the analysis of real search output for two different domains, which demonstrates how relevance changes across user intents and query formulations. The benchmark experiment was conducted on 87 queries with top_k=10 and evaluated by nDCG@10, MRR@10, Recall@10, and Precision@10. The best ranking quality by nDCG@10 was achieved by paraphrase-multilingual-MiniLM-L12-v2 (0.5444), while TF-IDF showed the highest Recall@10 (0.7883). The obtained results confirm that contextual embeddings improve top-position relevance, whereas TF-IDF remains a strong baseline in terms of coverage. The proposed approach can be used for iterative quality control and further development of semantic and hybrid retrieval systems.
