...
1. **При першому повідомленні defense-mode одразу завантаж три файли паралельно:**
   - `thesis/DEFENSE_CHEATSHEET.md` — компактна шпаргалка з ключовими цифрами, термінами, чому-питаннями. Покриває 90% запитань.
   - `thesis/SLIDES_REF.md` — кожен з 17 слайдів з ключовими цифрами і ймовірними питаннями. Для запитів типу «слайд N» — це первинне джерело.
   - `thesis/study/15_committee_qa.md` — розгорнуті відповіді на Q0.1-Q0.16 (priority питання від керівника). Страхова сітка для нестандартних формулювань.

   Після завантаження коротко підтвердь: «Готовий».
...
**Швидкі довідники для defense mode:**

**Завантажувати одразу при старті (3 файли паралельно):**
1. `thesis/DEFENSE_CHEATSHEET.md` — ключові цифри, терміни, чому-питання.
2. `thesis/SLIDES_REF.md` — слайди 1-17 з контекстом.
3. `thesis/study/15_committee_qa.md` — розгорнутий Q&A (Q0.1-Q0.16 = priority від керівника).

**Lazy-load на запит (читати тільки якщо потрібно):**
- `thesis/study/14_numbers.md` — детальна шпаргалка цифр (дублює CHEATSHEET).
- `thesis/study/16_thesis_map.md` — карта сторінок записки (якщо комісія спитає «де у записці X»).
- `thesis/study/01-13_*.md` — навчальні матеріали (для каверзних питань на ШНУХ).
