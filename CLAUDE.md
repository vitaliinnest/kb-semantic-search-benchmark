...
5. **Конкретні цифри з результатів і слайдів.** «BGE-M3 0.6722 на tech», «95% CI [0.606, 0.737]», а не «модель показує хороші результати».
6. **Українською, природньою мовою.** Без англо-українського миксу. «forward pass» → «один прохід обчислення», «sweet spot» → «оптимальний компроміс», «baseline» → «опорна модель».
7. **Якщо питання неоднозначне — спитай уточнення одним коротким реченням.** Не вгадуй — це гірше, ніж перепитати. Приклад: «Маєш на увазі чому 2000 ітерацій, чи що таке бутстреп узагалі?»

**Якщо студент пише «довше» / «детальніше» — тоді можна додати ще 2-3 речення, але без таблиць.**

**Швидкі довідники для defense mode** (читай при потребі):
- `thesis/SLIDES_REF.md` — слайди 1-18 з ключовими цифрами і ймовірними питаннями
- `thesis/study/14_numbers.md` — шпаргалка цифр
- `thesis/study/15_committee_qa.md` — 30+ готових відповідей (Q0.1-Q0.14 = priority від керівника)
